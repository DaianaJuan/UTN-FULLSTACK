...
function calcularIVA(precioSinIVA, porcentajeIVA) {
    let iva = precioSinIVA * (porcentajeIVA / 100);
    let precioConIVA = precioSinIVA + iva;
    return precioConIVA;
}

// Ejemplo de uso
let precio = parseFloat(prompt("Ingrese el precio sin IVA:"));
let porcentaje = parseFloat(prompt("Ingrese el porcentaje de IVA:"));

let precioFinal = calcularIVA(precio, porcentaje);
alert("El precio con IVA incluido es: " + precioFinal.toFixed(2));
*/
...
function validarNum(){
    ...
    while(!num || isNaN(num)){
        num = prompt('ERROR, vuelva a ingresar el número')
    }
    num = Number(num)
    return num
}
console.log(validarNum());
